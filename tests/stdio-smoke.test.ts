...
describe('stdio smoke (built dist)', () => {
  it('answers initialize and lists 9 tools', async () => {
  ...
  it('selfhost backend serves 9 tools and returns a reachable ephemeral-port URL', async () => {
    ...
    expect((JSON.parse(line(2)).result.tools as { name: string }[]).length).toBe(9);
  ...
  it('second selfhost client on a busy port keeps its tool layer alive (EADDRINUSE graceful)', async () => {
    ...
    expect((JSON.parse(toolsLine).result.tools as { name: string }[]).length).toBe(9);
  }, 15_000);
