...
export class SelfHostBackend implements ShareBackend {
  ...
  async createDoc(p: CreateDocParams): Promise<{ url: string }> {
    ...
    const dup = this.db.prepare(`SELECT docId FROM docs WHERE status = 'active' AND contentHash = ? AND createdAt > ?`).get(hash, windowStart) as { docId: string } | undefined;
  ...
  async searchDocs(p: SearchParams): Promise<DocRecord[]> {
    ...
    const rows = this.db.prepare(`
      ...
      p.titleQuery ?? '', p.titleQuery ?? '', p.status ?? null, p.status ?? null, limit) as never[];
